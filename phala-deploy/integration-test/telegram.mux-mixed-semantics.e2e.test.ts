      ...
      async () => {
        ...
        await withMuxOpenClawHarness(
          ...
          async (harness) => {
            ...
            expect(matchedSends).toHaveLength(2);
            expect(
              matchedSends.some((request) => String(request.body.text).includes(legacyReply)),
            ).toBe(true);
            expect(
              matchedSends.some((request) => String(request.body.text).includes(canonicalMessage)),
            ).toBe(true);
