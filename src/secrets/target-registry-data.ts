...
const CORE_SECRET_TARGET_REGISTRY: SecretTargetRegistryEntry[] = [
  ...
  {
    id: "mcp.servers.*.auth.token",
    targetType: "mcp.servers.auth.token",
    targetTypeAliases: ["mcp.servers.*.auth.token"],
    configFile: "openclaw.json",
    pathPattern: "mcp.servers.*.auth.token",
    secretShape: SECRET_INPUT_SHAPE,
    expectedResolvedValue: "string",
    includeInPlan: true,
    includeInConfigure: true,
  ...
  },
  {
    ...
    targetTypeAliases: ["mcp.servers.*.headers.*"],
    configFile: "openclaw.json",
