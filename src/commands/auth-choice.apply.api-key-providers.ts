...
const SIMPLE_API_KEY_PROVIDER_FLOWS: Partial<Record<AuthChoice, SimpleApiKeyProviderFlow>> = {
  ...
  "redpill-api-key": {
    ...
    expectedProviders: ["redpill"],
    envLabel: "REDPILL_API_KEY",
    promptMessage: "Enter Redpill AI API key",
    setCredential: setRedpillApiKey,
    ...
    noteMessage: [
      "Redpill AI provides privacy-preserving inference for GPU TEE models.",
      "Get your API key at: https://platform.redpill.ai/",
    ].join("\n"),
    noteTitle: "Redpill AI",
    normalize: (value) => String(value ?? "").trim(),
    validate: (value) => (String(value ?? "").trim() ? undefined : "Required"),
  },
};
