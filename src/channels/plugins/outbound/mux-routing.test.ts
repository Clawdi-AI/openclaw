...
describe("mux outbound routing", () => {
  ...
  it("routes imessage outbound through mux when enabled", async () => {
    const fetchSpy = vi.fn(async (input: string | URL | Request, _init?: RequestInit) => {
      const url = resolveFetchUrl(input);
      if (url === "http://mux.local/v1/instances/register") {
        return jsonResponse({
      ...
      if (url === "http://mux.local/v1/mux/outbound/send") {
        return jsonResponse({ messageId: "mx-im-1", chatId: "chat-guid-1" });
      }
      throw new Error(`unexpected url ${url}`);
    });
    globalThis.fetch = fetchSpy as unknown as typeof fetch;
  ...
  it("bypasses mux for imessage non-default accounts", async () => {
    const fetchSpy = vi.fn();
    globalThis.fetch = fetchSpy as unknown as typeof fetch;

    const sendIMessage = vi.fn().mockResolvedValue({
      messageId: "native-im-1",
    });
    const cfg = {
      ...gatewayMuxConfig(),
      channels: {
  ...
  it("routes imessage outbound through mux from default account config", async () => {
    const fetchSpy = vi.fn(async (input: string | URL | Request, _init?: RequestInit) => {
      const url = resolveFetchUrl(input);
      if (url === "http://mux.local/v1/instances/register") {
        return jsonResponse({
      ...
      if (url === "http://mux.local/v1/mux/outbound/send") {
        return jsonResponse({ messageId: "mx-im-acct-1" });
      }
      throw new Error(`unexpected url ${url}`);
    });
    globalThis.fetch = fetchSpy as unknown as typeof fetch;
