...
import { createTelegramStreamingDispatch } from "../telegram/streaming-dispatch.js";
...
function resolveTelegramMuxGroupConfig(params: {
  ...
  // Mux business routing may normalize inbound account ids onto the default
  // business path while operators still keep chat/topic policy under the
  // channel-level telegram.groups map.
  const groups = telegramCfg.groups ?? params.cfg.channels?.telegram?.groups;
  if (!groups) {
...
async function resolveTelegramMuxAccess(params: {
  ...
  const effectiveGroupAllow =
    runtimePairedSenders.length > 0
      ? normalizeAllowFromWithStore({
          allowFrom: [
            ...(groupAllowContext.groupAllowOverride ?? groupAllowFrom ?? []),
            ...runtimePairedSenders,
          ],
        })
      : groupAllowContext.effectiveGroupAllow;
...
export async function handleMuxInboundHttpRequest(
  ...
  const ctx: MsgContext = {
    ...
    ExplicitDeliverRoute: !isTelegramStreaming,
  ...
  const dispatchPromise = (async () => {
    try {
      ...
      if (channel === "telegram") {
        telegramAccess = callbackForwarded
          ? { allowed: true, commandAuthorized: true, effectiveWasMentioned: true }
          : await resolveTelegramMuxAccess({
              cfg,
              accountId,
              payload,
              channelData,
