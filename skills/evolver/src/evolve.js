...
function shouldSkipHubCalls(signals) {
  ...
  for (let si = 0; si < saturationIndicators.length; si++) {
    if (signals.indexOf(saturationIndicators[si]) !== -1) {
      hasSaturation = true;
      break;
    }
  }
...
try {
  ...
  console.warn(
    "[Evolver] Failed to create MEMORY_DIR (may cause downstream errors):",
    (e && e.message) || e,
  );
}
...
function computeAdaptiveStrategyPolicy(opts) {
  ...
  const antiPatterns =
    selectedGene && Array.isArray(selectedGene.anti_patterns)
      ? selectedGene.anti_patterns.slice(-5)
      : [];
  const learningHistory =
    selectedGene && Array.isArray(selectedGene.learning_history)
      ? selectedGene.learning_history.slice(-6)
      : [];
  const signalTags = new Set(expandSignals(signals, ""));
  const overlappingAntiPatterns = antiPatterns.filter(function (ap) {
    return (
      ap &&
      Array.isArray(ap.learning_signals) &&
      ap.learning_signals.some(function (tag) {
        return signalTags.has(String(tag));
      })
    );
  });
  const hardFailures = overlappingAntiPatterns.filter(function (ap) {
    return ap && ap.mode === "hard";
  }).length;
  const softFailures = overlappingAntiPatterns.filter(function (ap) {
    return ap && ap.mode !== "hard";
  }).length;
  const recentSuccesses = learningHistory.filter(function (x) {
    return x && x.outcome === "success";
  }).length;

  const stagnation =
    signals.includes("stable_success_plateau") ||
  ...
  if (forceInnovate)
    directives.push("Force strategy shift: prefer innovate over repeating repair/optimize.");
  if (highRiskGene)
    directives.push(
      "Selected gene is high risk for current signals; keep blast radius narrow and prefer smallest viable change.",
    );
  if (failureStreak >= 2)
    directives.push("Recent failure streak detected; avoid repeating recent failed approach.");
...
async function run() {
  ...
  verbose(
    "Config: EVOLVE_STRATEGY=" +
      (process.env.EVOLVE_STRATEGY || "(default)") +
      " EVOLVE_BRIDGE=" +
      (process.env.EVOLVE_BRIDGE || "(default)") +
      " EVOLVE_LOOP=" +
      (process.env.EVOLVE_LOOP || "false"),
  );
  verbose(
    "Config: EVOLVER_IDLE_FETCH_INTERVAL_MS=" +
      (process.env.EVOLVER_IDLE_FETCH_INTERVAL_MS || "(default 1800000)") +
      " RANDOM_DRIFT=" +
      (process.env.RANDOM_DRIFT || "false"),
  );
  ...
  if (shouldSkipHubCalls(signals)) {
    ...
    if (_lastHubFetchMs > 0 && _elapsed < _idleFetchInterval) {
      skipHubCalls = true;
      console.log(
        "[IdleGating] Saturated with no actionable signals. Skipping Hub API calls (last fetch " +
          Math.round(_elapsed / 1000) +
          "s ago, threshold " +
          Math.round(_idleFetchInterval / 1000) +
          "s).",
      );
    } else {
      console.log(
        "[IdleGating] Saturated but fetch interval elapsed (" +
          Math.round((Date.now() - _lastHubFetchMs) / 1000) +
          "s). Performing periodic Hub check.",
      );
    }
  }
  ...
  try {
    ...
    if (solidifyState && solidifyState.last_validation_failure) {
      ...
      console.log(
        "[RetryContext] Injected validation failure context from previous solidify (retries=" +
          (lvf.retries_attempted || 0) +
          ").",
      );
    }
  } catch (_) {}
  ...
  try {
    ...
    var {
      getNoveltyHint: _getNoveltyHintEarly,
      getCapabilityGaps: _getCapGapsEarly,
    } = require("./gep/a2aProtocol");
    var earlyCapGaps = [];
    try {
      earlyCapGaps = _getCapGapsEarly() || [];
    } catch (_) {}
    var memGraphPath = require("./gep/memoryGraph").memoryGraphPath
      ? require("./gep/memoryGraph").memoryGraphPath()
      : "";
    ...
    try {
      ...
      if (hubTasks.length > 0) {
        ...
        try {
          ...
          console.warn(
            "[TaskReceiver] MemoryGraph read failed (task selection proceeds without history):",
            (e && e.message) || e,
          );
        }
        ...
        if (best) {
          ...
          if (!alreadyClaimed) {
            ...
            claimed = await claimTask(
              ...
              commitDeadline ? { commitment_deadline: commitDeadline } : undefined,
            );
    ...
    if (overdueTasks.length > 0) {
      for (const ot of overdueTasks) {
        ...
        if (activeTask && (activeTask.id === otId || activeTask.task_id === otId)) {
          console.warn(
            `[Commitment] Active task "${activeTask.title || otId}" is OVERDUE -- prioritizing completion.`,
          );
    ...
    try {
      ...
      if (workerTasks.length > 0) {
        ...
        try {
          ...
          console.warn(
            "[WorkerPool] MemoryGraph read failed (task selection proceeds without history):",
            (e && e.message) || e,
          );
        }
        ...
        if (best) {
          ...
          console.log(
            `[WorkerPool] Selected worker task (deferred claim): "${best.title || best.id}" (${taskSignals.length} signals injected)`,
          );
        }
      }
    } catch (e) {
  ...
  const { selectedGene, capsuleCandidates, selector } = selectGeneAndCapsule({
    ...
    noveltyScore:
      heartbeatNovelty && Number.isFinite(heartbeatNovelty.score) ? heartbeatNovelty.score : null,
  });
  ...
  verbose(
    "Gene selection: gene=" +
      (selectedGene ? selectedGene.id : "(none)") +
      " capsule=" +
      (selectedCapsuleId || "(none)") +
      " selectedBy=" +
      selectedBy +
      " selector=" +
      (selector || "(none)"),
  ...
  verbose(
    "Strategy policy: name=" +
      strategyPolicy.name +
      " forceInnovate=" +
      strategyPolicy.forceInnovate +
      " cautious=" +
      strategyPolicy.cautiousExecution +
      " maxFiles=" +
      strategyPolicy.blastRadiusMaxFiles,
  );
  if (memoryAdvice) {
    verbose(
      "Memory advice: preferred=" +
        (memoryAdvice.preferredGeneId || "(none)") +
        " banned=[" +
        (Array.isArray(memoryAdvice.bannedGeneIds) ? memoryAdvice.bannedGeneIds.join(",") : "") +
        "]",
    );
  }
  ...
  const mutationSignals = innovationPressure
    ...
    : signals;
  const mutationSignalsEffective =
    forceInnovation || strategyPolicy.forceInnovate
  ...
  verbose(
    "Hub: hubHit=" +
      (hubHit && hubHit.hit
        ? "true (score=" + hubHit.score + " mode=" + hubHit.mode + ")"
        : "false (" + ((hubHit && hubHit.reason) || "unknown") + ")"),
  );
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "[SolidifyState] Failed to read baseline untracked files:",
        (e && e.message) || e,
      );
    }
    ...
    const maxFiles =
      strategyPolicy && Number.isFinite(Number(strategyPolicy.blastRadiusMaxFiles))
