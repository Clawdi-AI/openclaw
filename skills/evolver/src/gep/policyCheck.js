...
function checkConstraints({ gene, blast, blastRadiusEstimate, repoRoot }) {
  ...
  const allowSelfModify =
    String(process.env.EVOLVE_ALLOW_SELF_MODIFY || "").toLowerCase() === "true";
  for (const f of blast.all_changed_files || blast.changed_files || []) {
  ...
  if (repoRoot) {
    ...
    newSkillDirs.forEach(function (skillName) {
      ...
      try {
        const entries = fs.readdirSync(skillDir).filter(function (e) {
          return !e.startsWith(".");
        });
        if (entries.length < 2) {
          warnings.push(
        ...
        console.warn(
          "[policyCheck] checkConstraints skill dir read failed:",
          skillName,
          (e && e.message) || e,
        );
      }
    ...
    if (!baselineSet.has(norm)) {
      if (!fs.existsSync(normAbs)) {
        ...
        try {
          ...
          console.warn(
            "[policyCheck] detectDestructiveChanges stat failed:",
            norm,
            (e && e.message) || e,
          );
        }
...
function runValidations(gene, opts = {}) {
  ...
  while (attempt <= maxRetries) {
    ...
    var blocked =
      result.results &&
      result.results.some(function (r) {
    ...
    if (attempt <= maxRetries) {
      console.log(
        "[Solidify] Validation failed (attempt " +
          attempt +
          "/" +
          (maxRetries + 1) +
          "), retrying in 1s...",
...
function buildSoftFailureLearningSignals(opts) {
  ...
  const validationResults =
    opts && Array.isArray(opts.validationResults) ? opts.validationResults : [];
  const validationText = validationResults
    .filter(function (r) {
      return r && r.ok === false;
    })
    .map(function (r) {
      return [r.cmd, r.stderr, r.stdout].filter(Boolean).join(" ");
    })
    .join(" ");
  return expandSignals(signals.concat(violations), failureReason + " " + validationText).filter(
    function (tag) {
      return (
        tag.indexOf("problem:") === 0 ||
        tag.indexOf("risk:") === 0 ||
        tag.indexOf("area:") === 0 ||
...
function classifyFailureMode(opts) {
  const constraintViolations =
    opts && Array.isArray(opts.constraintViolations) ? opts.constraintViolations : [];
  const protocolViolations =
    opts && Array.isArray(opts.protocolViolations) ? opts.protocolViolations : [];
  const validation = opts && opts.validation ? opts.validation : null;
  const canary = opts && opts.canary ? opts.canary : null;

  if (
    constraintViolations.some(function (v) {
      const s = String(v || "");
      return /HARD CAP BREACH|CRITICAL_FILE_|critical_path_modified|forbidden_path touched|ethics:/i.test(
        s,
      );
    })
  ) {
