...
function getEpigeneticBoostLocal(gene, envFingerprint) {
  ...
  const mark = gene.epigenetic_marks.find(function (m) {
    return m && m.context === envContext;
  });
...
function scoreGeneLearning(gene, signals, envFingerprint) {
  ...
  if (Array.isArray(gene.anti_patterns) && gene.anti_patterns.length > 0) {
    ...
    for (let j = 0; j < recentAntiPatterns.length; j++) {
      ...
      const overlap = anti.learning_signals.some(function (tag) {
...
function selectGene(genes, signals, opts) {
  ...
  const noveltyScore =
    opts && Number.isFinite(Number(opts.noveltyScore)) ? Number(opts.noveltyScore) : null;
    ...
    if (capabilityGaps.length > 0) {
      ...
      const gapScores = filtered.map(function (entry, idx) {
        ...
        for (let gi = 0; gi < capabilityGaps.length && gi < 5; gi++) {
          ...
          if (
            typeof gapSignal === "string" &&
            patterns.some(function (p) {
              return matchPatternToSignals(p, [gapSignal]);
            })
          ) {
            gapHits++;
          }
      ...
      const hasGapHits = gapScores.some(function (gs) {
        return gs.gapHits > 0;
      });
      if (hasGapHits) {
