...
function buildSemanticQuery(signals) {
  return signals
    .filter(function (s) {
      return !s.startsWith("errsig:") && !s.startsWith("errsig_norm:");
    })
    .map(function (s) {
...
async function fetchSemanticResults(hubUrl, headers, signalList, timeoutMs) {
  ...
  var url =
    hubUrl + "/a2a/assets/semantic-search?q=" + encodeURIComponent(query) + "&type=Gene&limit=10";
  ...
  for (var j = 0; j < semanticResults.length; j++) {
    ...
    if (bid && seen[bid]) {
      var existing = merged.find(function (m) {
        return (m.asset_id || m.assetId) === bid;
      });
...
async function hubSearch(signals, opts) {
  ...
  try {
    ...
    if (!results) {
      ...
      var fetchResult =
        settled[0].status === "fulfilled" ? settled[0].value : { ok: false, results: [] };
        ...
        if (remaining > MIN_PHASE2_MS) {
          try {
            ...
            if (res2.ok) {
              ...
              const fullResults =
                data2 && data2.payload && Array.isArray(data2.payload.results)
          ...
          console.log(
            `[HubSearch] Phase 2 skipped: ${remaining}ms remaining < ${MIN_PHASE2_MS}ms threshold`,
          );
        }
      }
    }

    var viaLabel = cacheHit
      ? "search_cached"
      : semanticUsed
        ? "search+semantic"
        : "search_then_fetch";
    console.log(
      `[HubSearch] Hit via ${viaLabel}: ${pick.match.asset_id || "unknown"} (score=${pick.score}, mode=${pick.mode}${pick.match._semantic_similarity ? ", sim=" + pick.match._semantic_similarity : ""})`,
    );
