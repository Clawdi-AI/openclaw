...
const DISTILLER_MIN_SUCCESS_RATE =
...
const FAILURE_DISTILLER_MIN_CAPSULES =
  parseInt(process.env.FAILURE_DISTILLER_MIN_CAPSULES || "5", 10) || 5;
const FAILURE_DISTILLER_INTERVAL_HOURS =
  parseInt(process.env.FAILURE_DISTILLER_INTERVAL_HOURS || "12", 10) || 12;
...
function collectDistillationData() {
  ...
  const successCapsules = allCapsules.filter(function (c) {
    ...
    const score =
      c.outcome && Number.isFinite(Number(c.outcome.score)) ? Number(c.outcome.score) : 1;
...
function buildDistillationPrompt(analysis, existingGenes, sampleCapsules) {
  ...
  return [
    ...
    '- The id MUST start with "' +
      DISTILLED_ID_PREFIX +
      '" followed by a descriptive kebab-case name.',
...
function sanitizeSignalsMatch(signals) {
  ...
  signals.forEach(function (s) {
    ...
    if (
      /^(cursor|vscode|vim|emacs|windsurf|copilot|cline|codex|bypass|distill)[_-]?\d*$/i.test(sig)
    )
      return;
  ...
  // Deduplicate
  const seen = {};
  return cleaned.filter(function (s) {
...
function inferCategoryFromSignals(signals) {
  ...
  if (
    list.some(function (s) {
      return (
        s.indexOf("error") !== -1 || s.indexOf("fail") !== -1 || s.indexOf("reliability") !== -1
      );
    })
  ) {
    return "repair";
...
function synthesizeGeneFromPatterns(data, analysis, existingGenes) {
  ...
  const sourceGene =
    existing.find(function (g) {
      return g && g.id === source.gene_id;
    }) || null;
  ...
  let signalsMatch = Object.keys(triggerFreq)
    .sort(function (a, b) {
      return triggerFreq[b] - triggerFreq[a];
    })
    .slice(0, 6);
  const summaryText = (group.summaries || []).slice(0, 5).join(" ");
  const derivedTags = learningSignals
    .expandSignals(signalsMatch, summaryText)
    .filter(function (tag) {
      return tag.indexOf("problem:") === 0 || tag.indexOf("area:") === 0;
    })
    .slice(0, 4);
  ...
  const category =
    sourceGene && sourceGene.category
      ? sourceGene.category
      : inferCategoryFromSignals(signalsMatch);
  const idSeed = {
    type: "Gene",
    ...
    strategy:
      sourceGene && Array.isArray(sourceGene.strategy) && sourceGene.strategy.length > 0
  ...
  if (!summaryBase) {
    summaryBase =
      "Reusable strategy for repeated successful pattern: " + signalsMatch.slice(0, 3).join(", ");
  }
  ...
  const gene = {
    ...
    signals_match: signalsMatch,
    preconditions:
      sourceGene && Array.isArray(sourceGene.preconditions) && sourceGene.preconditions.length > 0
        ? sourceGene.preconditions.slice(0, 4)
        : ["repeated success pattern observed in recent capsules"],
    strategy: idSeed.strategy,
    constraints: {
      max_files:
        sourceGene && sourceGene.constraints && Number(sourceGene.constraints.max_files) > 0
          ? Math.min(DISTILLED_MAX_FILES, Number(sourceGene.constraints.max_files))
          : DISTILLED_MAX_FILES,
      forbidden_paths:
        sourceGene &&
        sourceGene.constraints &&
        Array.isArray(sourceGene.constraints.forbidden_paths)
          ? sourceGene.constraints.forbidden_paths.slice(0, 6)
          : [".git", "node_modules"],
    },
    validation:
      sourceGene && Array.isArray(sourceGene.validation) && sourceGene.validation.length > 0
    ...
    try {
      ...
      skillPublisher
        ...
        .then(function (res) {
          if (res.ok) {
            console.log(
              "[Distiller] Auto-distilled skill published: " +
                ((res.result && res.result.skill_id) || gene.id),
            );
          } else {
            console.warn(
              "[Distiller] Auto-distilled skill publish failed: " + (res.error || "unknown"),
            );
          }
        })
        .catch(function () {});
...
function collectFailureDistillationData() {
  ...
  const failedCapsules = Array.isArray(failedData.failed_capsules)
    ? failedData.failed_capsules
    : [];
  ...
  failedCapsules.forEach(function (c) {
    if (!c) return;
    const reasonClass =
      (c.failure_reason || "").split(":")[0].split(" ")[0].toLowerCase() || "unknown";
...
function analyzeFailurePatterns(data) {
  ...
  Object.keys(grouped).forEach(function (key) {
    ...
    if (g.count >= 2) {
      ...
      var topTriggers = Object.keys(freq)
        .sort(function (a, b) {
          return freq[b] - freq[a];
        })
        .slice(0, 5);
      ...
      var topViolations = Object.keys(violationFreq)
        .sort(function (a, b) {
          return violationFreq[b] - violationFreq[a];
        })
        .slice(0, 3);
  ...
  report.recurring_violations = Object.keys(allViolations)
    .filter(function (k) {
      return allViolations[k] >= 2;
    })
    .sort(function (a, b) {
      return allViolations[b] - allViolations[a];
    })
    .slice(0, 8)
    .map(function (k) {
      return { violation: k, count: allViolations[k] };
    });
...
function buildFailureDistillationPrompt(analysis, existingGenes, sampleCapsules) {
  ...
  return [
    ...
    '{ "type": "Gene", "id": "' +
      REPAIR_DISTILLED_ID_PREFIX +
      '<descriptive-kebab-name>", "summary": "<defensive description>", "category": "repair", "signals_match": ["signal_1", ...], "preconditions": ["condition 1", ...], "strategy": ["Step 1: guard ...", "Step 2: verify ...", ...], "constraints": { "max_files": N, "forbidden_paths": [".git", "node_modules", ...] }, "validation": ["npx tsc --noEmit", ...], "schema_version": "1.6.0" }',
  ].join("\n");
}
...
function synthesizeRepairGeneFromFailures(data, analysis, existingGenes) {
  if (
    !analysis ||
    !analysis.high_frequency_failures ||
    analysis.high_frequency_failures.length === 0
  )
    return null;
  ...
  if (violationSample) {
    strategy.splice(
      2,
      0,
      'GUARD: Previous failures involved "' +
        violationSample +
        '" -- add explicit checks to prevent recurrence.',
    );
  }
  ...
  var gene = {
    ...
    signals_match: signalsMatch,
    preconditions: [
      "Repeated failure pattern detected in recent capsules (" + topPattern.count + " occurrences)",
    ],
    strategy: strategy,
    constraints: {
...
function autoDistillFromFailures() {
  ...
  console.log(
    "[Distiller] Repair gene distilled from " +
      data.failedCapsules.length +
      " failures: " +
      gene.id,
  );
