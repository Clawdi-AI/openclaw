...
function extractCapabilityCandidates({ recentSessionTranscript, signals, recentFailedCapsules }) {
  ...
  for (var i = 0; i < failedCapsules.length; i++) {
    ...
    var failureTags = expandSignals((fc.trigger || []).concat(signalList), reason).filter(
      function (t) {
        return (
          t.indexOf("problem:") === 0 ||
          t.indexOf("risk:") === 0 ||
    ...
    var groupingTags = dominantProblem
      ...
      : failureTags
          .filter(function (tag) {
            return tag.indexOf("area:") === 0 || tag.indexOf("risk:") === 0;
          })
          .slice(0, 1);
    var key = groupingTags.join("|");
    if (!groups[key])
      groups[key] = { count: 0, tags: failureTags, reasons: [], gene: fc.gene || null };
    groups[key].count += 1;
  ...
  Object.keys(groups).forEach(function (key) {
    ...
    if (group.tags.indexOf("problem:performance") !== -1)
      title = "Resolve recurring performance regressions";
    else if (group.tags.indexOf("problem:protocol") !== -1)
      title = "Prevent recurring protocol and validation regressions";
    else if (group.tags.indexOf("problem:reliability") !== -1)
      title = "Repair recurring reliability failures";
    else if (group.tags.indexOf("problem:stagnation") !== -1)
      title = "Break repeated stagnation loops with a new strategy";
    else if (group.tags.indexOf("area:orchestration") !== -1)
      title = "Stabilize task and orchestration behavior";
    var evidence =
      "Observed " +
      group.count +
      " recent failed evolutions with similar learning tags. " +
