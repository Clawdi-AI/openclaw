...
function writeStateForSolidify(state) {
  ...
  try {
    ...
    console.warn(
      "[evolver] writeStateForSolidify mkdir failed:",
      evolutionDir,
      (e && e.message) || e,
    );
  }
...
function adaptGeneFromLearning(opts) {
  ...
  const failureMode =
    opts && opts.failureMode && typeof opts.failureMode === "object"
      ? opts.failureMode
...
function computeProcessScores(opts) {
  ...
  if (blast) {
    const maxFiles =
      geneUsed && geneUsed.constraints && geneUsed.constraints.max_files
        ? geneUsed.constraints.max_files
        : 12;
    if (blast.files === 0) {
      blastScore = 0.4;
  ...
  if (constraintCheck && !constraintCheck.ok) {
    const violationCount = Array.isArray(constraintCheck.violations)
      ? constraintCheck.violations.length
      : 0;
  ...
  if (validation && Array.isArray(validation.results) && validation.results.length > 0) {
    const passed = validation.results.filter(function (r) {
      return r && r.ok;
    }).length;
...
function solidify({ intent, summary, dryRun = false, rollbackOnFailure = true } = {}) {
  ...
  if (
    ...
    try {
      ...
      if (llmReviewResult && llmReviewResult.approved === false) {
        constraintCheck.violations.push(
          "llm_review_rejected: " + (llmReviewResult.summary || "no reason"),
        );
        constraintCheck.ok = false;
        console.log("[LLMReview] Change REJECTED: " + (llmReviewResult.summary || ""));
      } else if (llmReviewResult) {
        console.log(
          "[LLMReview] Change approved (confidence: " + (llmReviewResult.confidence || "?") + ")",
        );
      }
    } catch (e) {
  ...
  const failureReason = !success
    ? buildFailureReason(constraintCheck, validation, protocolViolations, canary)
    : "";
  ...
  if (success) {
    ...
    const capsuleStrategy =
      geneUsed && Array.isArray(geneUsed.strategy) && geneUsed.strategy.length > 0
        ? geneUsed.strategy
        : undefined;
    capsule = {
      type: "Capsule",
  ...
  if (!success && validation && !validation.ok) {
    var failedCmd =
      validation.results &&
      validation.results.find(function (r) {
        return !r.ok;
      });
    ...
    if (resultAssetId) {
      ...
      if (workerPending && !workerAssignmentId) {
        ...
        try {
          ...
          console.log(
            `[WorkerPool] Atomic claim+complete for task "${lastRun.active_task_title || taskId}" with asset ${resultAssetId}`,
          );
          ...
          if (result && typeof result.then === "function") {
            result
              .then(function (r) {
                if (r.ok) {
                  console.log(
                    "[WorkerPool] Claim+complete succeeded, assignment=" + r.assignment_id,
                  );
                } else {
                  console.log(
                    "[WorkerPool] Claim+complete failed: " +
                      (r.error || "unknown") +
                      (r.assignment_id ? " assignment=" + r.assignment_id : ""),
                  );
                }
              })
              .catch(function (err) {
                console.log(
                  "[WorkerPool] Claim+complete error (non-fatal): " +
    ...
    try {
      ...
      if (hubReviewPromise && typeof hubReviewPromise.then === "function") {
        hubReviewPromise = hubReviewPromise
          ...
          .catch(function (err) {
            console.log(
              "[HubReview] Error (non-fatal): " + (err && err.message ? err.message : err),
            );
            return null;
          });
      }
