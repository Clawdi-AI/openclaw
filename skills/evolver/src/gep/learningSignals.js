...
function expandSignals(signals, extraText) {
  const raw = Array.isArray(signals)
  ...
  if (
    /(feature|capability_gap|user_feature_request|external_opportunity|stagnation recommendation)/.test(
      text,
    )
  ) {
  ...
  if (
    /(stagnation|plateau|steady_state|saturation|empty_cycle_loop|loop_detected|recurring)/.test(
      text,
    )
  ) {
