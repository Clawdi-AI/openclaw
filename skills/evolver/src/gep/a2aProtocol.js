...
function fileTransportReceive(opts) {
  ...
  const files = fs.readdirSync(subdir).filter(function (f) {
    ...
    try {
      ...
      for (let li = 0; li < lines.length; li++) {
        try {
          ...
          console.warn(
            "[a2aProtocol] Malformed JSON line in inbox file " +
              files[fi] +
              " (line " +
              (li + 1) +
              "):",
...
function sendHelloToHub() {
  ...
  return fetch(endpoint, {
    ...
    .then(function (data) {
      const secret =
        (data && data.payload && data.payload.node_secret) || (data && data.node_secret) || null;
  ...
  return fetch(endpoint, {
    ...
    .then(function (data) {
      if (data && (data.error === "rate_limited" || data.status === "rate_limited")) {
        ...
        const backoff =
          retryMs > 0 ? retryMs + 5000 : windowMs > 0 ? windowMs + 5000 : _heartbeatIntervalMs;
      ...
      if (Array.isArray(data.overdue_tasks) && data.overdue_tasks.length > 0) {
        ...
        console.warn(
          "[Commitment] " + data.overdue_tasks.length + " overdue task(s) detected via heartbeat.",
        );
      }
      if (data.skill_store) {
      ...
      if (data.circle_experience && typeof data.circle_experience === "object") {
        console.log(
          "[EvolutionCircle] Active circle: " +
            (data.circle_experience.circle_id || "?") +
            " (" +
            (data.circle_experience.member_count || 0) +
            " members)",
        );
      }
      _heartbeatConsecutiveFailures = 0;
