...
function estimateCommitmentDeadline(task) {
  ...
  var difficulty =
    task.complexity_score != null ? Number(task.complexity_score) : localDifficultyEstimate(task);
...
function scoreTask(task, capabilityMatch) {
  ...
  var difficulty =
    task.complexity_score != null ? task.complexity_score : localDifficultyEstimate(task);
  var bountyAmount = task.bounty_amount || 0;
  var completionRate =
    task.historical_completion_rate != null ? task.historical_completion_rate : 0.5;
