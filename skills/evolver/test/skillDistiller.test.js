...
describe("validateSynthesizedGene", () => {
  it("accepts a valid gene", () => {
    var gene = {
      ...
      category: "repair",
      signals_match: ["error"],
      strategy: ["identify the root cause", "apply the fix", "verify the solution"],
  ...
  it("strips node -e and node --eval commands (consistent with policyCheck)", () => {
    var gene = {
      type: "Gene",
      id: "gene_distilled_eval_block",
      category: "opt",
      signals_match: ["test_signal"],
      strategy: ["step one", "step two", "step three"],
      constraints: { forbidden_paths: [".git", "node_modules"] },
      validation: [
...
describe("prepareDistillation", () => {
  ...
  it("returns idempotent_skip after completeDistillation with same data", () => {
    ...
    var llmResponse = JSON.stringify({
      ...
      category: "repair",
      signals_match: ["error"],
      strategy: ["identify root cause", "apply targeted fix", "verify correction"],
  ...
  it("builds a conservative distilled gene from repeated successful capsules", () => {
    writeCapsules([
      makeCapsule(
        ...
        "success",
        0.95,
        ["perf_bottleneck", "latency"],
        "Reduced latency in hot path",
      ),
      makeCapsule(
        "c2",
        "gene_perf",
      ...
      makeCapsule(
        ...
        0.96,
        ["perf_bottleneck"],
        "Stabilized latency under peak load",
      ),
      makeCapsule(
        "c7",
        "gene_perf",
      ...
      makeCapsule(
        ...
        0.99,
        ["perf_bottleneck"],
        "Improved repeated performance pattern",
      ),
      makeCapsule(
        "c10",
        "gene_perf",
        ...
        0.91,
        ["perf_bottleneck"],
        "Kept repeated success on perf fixes",
      ),
    ]);
    writeGenes([
      {
        ...
        strategy: [
          ...
          "Apply the narrowest optimization",
          "Run focused perf validation",
        ],
        constraints: { max_files: 8, forbidden_paths: [".git", "node_modules"] },
    ...
    var gene = synthesizeGeneFromPatterns(data, analysis, [
      { id: "gene_perf", category: "optimize", signals_match: ["perf_bottleneck"] },
    ]);
    assert.ok(gene);
    ...
    for (var i = 0; i < 10; i++) {
      caps.push(
        makeCapsule(
          ...
          0.95,
          ["perf_bottleneck"],
          "Reduce repeated latency regressions",
        ),
      );
    }
    writeCapsules(caps);
