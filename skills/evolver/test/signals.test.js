...
describe("extractSignals -- user_feature_request (4 languages)", () => {
  ...
  it("user_feature_request signal carries snippet", () => {
    ...
    assert.ok(
      extra.toLowerCase().includes("dark") || extra.includes("toggle") || extra.includes("add"),
...
describe("extractSignals -- windows_shell_incompatible", () => {
  ...
  it("detects pgrep on win32", () => {
    ...
    try {
      ...
      assert.ok(
        hasSignal(r, "windows_shell_incompatible"),
        "expected windows_shell_incompatible for pgrep on win32",
      );
    } finally {
      restorePlatform();
    }
  ...
  it("detects ps aux on win32", () => {
    ...
    try {
      ...
      assert.ok(
        hasSignal(r, "windows_shell_incompatible"),
        "expected windows_shell_incompatible for ps aux on win32",
      );
    } finally {
      restorePlatform();
    }
  ...
  it("detects cat > redirect on win32", () => {
    ...
    try {
      ...
      assert.ok(
        hasSignal(r, "windows_shell_incompatible"),
        "expected windows_shell_incompatible for cat > on win32",
      );
    } finally {
      restorePlatform();
    }
  ...
  it("detects heredoc on win32", () => {
    ...
    try {
      ...
      assert.ok(
        hasSignal(r, "windows_shell_incompatible"),
        "expected windows_shell_incompatible for heredoc on win32",
      );
    } finally {
      restorePlatform();
    }
  ...
  it("is treated as cosmetic and dropped when actionable signals exist", () => {
    ...
    try {
      ...
      assert.ok(
        !hasSignal(r, "windows_shell_incompatible"),
        "cosmetic signal should be dropped when actionable signals exist, got: " +
          JSON.stringify(r),
      );
    } finally {
      restorePlatform();
