...
describe("shouldSkipHubCalls", () => {
  ...
  it("returns true when only saturation signals are present", () => {
    assert.equal(
      shouldSkipHubCalls(["force_steady_state", "evolution_saturation", "stable_success_plateau"]),
      true,
    );
  });
  ...
  it("returns true for empty_cycle_loop_detected + stable_success_plateau", () => {
    assert.equal(shouldSkipHubCalls(["empty_cycle_loop_detected", "stable_success_plateau"]), true);
  });

  it("returns false when log_error coexists with saturation", () => {
    assert.equal(
      shouldSkipHubCalls(["force_steady_state", "evolution_saturation", "log_error"]),
      false,
    );
  });
  ...
  it("returns false when errsig: coexists with saturation", () => {
    assert.equal(
      shouldSkipHubCalls(["evolution_saturation", "errsig:TypeError: foo is not a function"]),
      false,
    );
  });

  it("returns false when recurring_error coexists with saturation", () => {
    assert.equal(shouldSkipHubCalls(["force_steady_state", "recurring_error"]), false);
  });

  it("returns false when external_task coexists with saturation", () => {
    assert.equal(
      shouldSkipHubCalls(["force_steady_state", "evolution_saturation", "external_task"]),
      false,
    );
  });

  it("returns false when bounty_task coexists with saturation", () => {
    assert.equal(shouldSkipHubCalls(["evolution_saturation", "bounty_task"]), false);
  });

  it("returns false when overdue_task coexists with saturation", () => {
    assert.equal(shouldSkipHubCalls(["force_steady_state", "overdue_task"]), false);
  });

  it("returns false when urgent coexists with saturation", () => {
    assert.equal(shouldSkipHubCalls(["evolution_saturation", "urgent"]), false);
  });

  it("returns false when capability_gap coexists with saturation", () => {
    assert.equal(shouldSkipHubCalls(["force_steady_state", "capability_gap"]), false);
  });

  it("returns false when perf_bottleneck coexists with saturation", () => {
    assert.equal(shouldSkipHubCalls(["empty_cycle_loop_detected", "perf_bottleneck"]), false);
  });

  it("returns false when user_feature_request has content", () => {
    assert.equal(
      shouldSkipHubCalls(["force_steady_state", "user_feature_request:add dark mode"]),
      false,
    );
  });

  it("returns true when user_feature_request is empty (no real request)", () => {
    assert.equal(shouldSkipHubCalls(["force_steady_state", "user_feature_request:"]), true);
  });

  it("returns false when user_improvement_suggestion has content", () => {
    assert.equal(
      shouldSkipHubCalls(["evolution_saturation", "user_improvement_suggestion:refactor the API"]),
      false,
    );
  });
  ...
  it("returns true when user_improvement_suggestion is empty", () => {
    assert.equal(
      shouldSkipHubCalls(["evolution_saturation", "user_improvement_suggestion:"]),
      true,
    );
  });

  it("returns false when unsupported_input_type coexists with saturation", () => {
    assert.equal(shouldSkipHubCalls(["force_steady_state", "unsupported_input_type"]), false);
  });
