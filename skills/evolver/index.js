...
try {
  require("dotenv").config({ path: path.join(getRepoRoot(), ".env") });
} catch (e) {
...
async function main() {
  ...
  if (!command || command === "run" || command === "/evolve" || isLoop) {
    if (isLoop) {
      ...
      console.log = (...args) => {
        originalLog.call(console, ts(), ...args);
      };
      console.warn = (...args) => {
        originalWarn.call(console, ts(), ...args);
      };
      console.error = (...args) => {
        originalError.call(console, ts(), ...args);
      };
    }
    ...
    if (isLoop) {
      ...
      console.log(
        `Loop mode enabled (internal daemon, bridge=${process.env.EVOLVE_BRIDGE}, verbose=${isVerbose}).`,
      );
      ...
      while (true) {
        try {
          ...
          try {
            ...
            if (schedule.enabled && schedule.sleep_multiplier > 0) {
              ...
              if (schedule.should_distill) {
                try {
                  const {
                    shouldDistillFromFailures: shouldDF,
                    autoDistillFromFailures: autoDF,
              ...
              if (isVerbose && schedule.idle_seconds >= 0) {
                console.log(
                  `[OMLS] idle=${schedule.idle_seconds}s intensity=${schedule.intensity} multiplier=${omlsMultiplier}`,
                );
              }
            }
          } catch (e) {}
          ...
          const totalSleepMs = Math.max(
            minSleepMs,
            (currentSleepMs + jitter) * saturationMultiplier * omlsMultiplier,
          );
          if (isVerbose) {
            const memMb = (process.memoryUsage().rss / 1024 / 1024).toFixed(1);
            console.log(
              `[Verbose] cycle=${cycleCount} ok=${ok} dt=${dt}ms sleep=${totalSleepMs}ms (base=${currentSleepMs} jitter=${jitter} sat=${saturationMultiplier}x) rss=${memMb}MB signals=[${(function () {
                try {
                  var st = readJsonSafe(solidifyStatePath);
                  return st && st.last_run && Array.isArray(st.last_run.signals)
                    ? st.last_run.signals.join(",")
                    : "";
                } catch (e) {
                  return "";
                }
        ...
        try {
            ...
            if (auto && auto.ok && auto.gene) {
              ...
              if (dr && dr.ok && dr.promptPath) {
                const trigger = autoTrigger
                  ? `auto (every ${autoDistillInterval} solidifies, count=${count})`
                  : "threshold";
                console.log("\n[DISTILL_REQUEST]");
          ...
          if (shouldDistillFromFailures()) {
            ...
            if (failureResult && failureResult.ok && failureResult.gene) {
              console.log(
                "[Distiller] Repair gene distilled from failures: " + failureResult.gene.id,
              );
            }
          }
        } catch (e) {
    ...
    let skillId = null;
    const eqFlag = args.find(
      (a) => typeof a === "string" && (a.startsWith("--skill=") || a.startsWith("-s=")),
    );
    if (eqFlag) {
    ...
    try {
      if (!getHubNodeSecret()) {
        ...
        if (!helloResult || !helloResult.ok) {
          console.error(
            "[fetch] Failed to register with Hub:",
            (helloResult && helloResult.error) || "unknown",
          );
          process.exit(1);
        }
      ...
      if (!resp.ok) {
        ...
        if (resp.status === 404) {
          ...
          console.error(
            "  Check the skill ID spelling, or browse available skills at https://evomap.ai",
          );
      ...
      const outDir = outFlag ? outFlag.slice("--out=".length) : path.join(".", "skills", safeId);
      ...
      console.log(
        "  Files:   SKILL.md" +
          (bundled.length > 0 ? ", " + bundled.map((f) => f.name).join(", ") : ""),
      );
      ...
      if (error && error.name === "TimeoutError") {
        ...
        if (error && error.cause)
          console.error("  Cause: " + (error.cause.message || error.cause.code || error.cause));
