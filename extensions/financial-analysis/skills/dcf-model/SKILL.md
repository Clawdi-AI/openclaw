...
- Default to using all of the information provided by the user and `financial_analysis_connectors` when it exposes a relevant data source.
...
Fetch data from `financial_analysis_connectors`, user provided data, and the web.

**Data Sources Priority:**

1. **`financial_analysis_connectors`** (if configured) - Structured financial data from providers like Daloopa
...
1. **Gather market data**:
   - Check `financial_analysis_connectors` for current market data sources
...
2. **Gather historical financials**:
   - Check `financial_analysis_connectors` for Daloopa or similar financial statement sources
   - Request from user if not available through the connector tool
...
- **`financial_analysis_connectors`**: If configured (for example Daloopa for historical financials)
