...
1. **FIRST: Check `financial_analysis_connectors`** - If S&P Kensho, FactSet, or Daloopa style connectors are configured there, use them exclusively for financial and trading information
2. **DO NOT use web search** if the above connector-backed sources are available
3. **ONLY if those connectors are unavailable:** Then use Bloomberg Terminal, SEC EDGAR filings, or other institutional sources
...
**Why this matters:** Connector-backed data sources provide verified, institutional-grade data with proper citations. Web search results can be outdated, inaccurate, or unreliable for financial analysis.
...
- Where did the data come from? (`financial_analysis_connectors` via S&P Kensho / FactSet / Daloopa, Bloomberg, SEC filings)
- What period does it cover? (Q4 2024, audited figures)
- How was it verified? (Cross-checked against 10-K/10-Q)
- Note: Prioritize `financial_analysis_connectors` entries for S&P Kensho, FactSet, and Daloopa when available for better accuracy and traceability
...
2. **Gather data** (60-90 minutes)
   - Pull from primary sources (`financial_analysis_connectors` for S&P Kensho, FactSet, or Daloopa when available; otherwise Bloomberg, SEC)
